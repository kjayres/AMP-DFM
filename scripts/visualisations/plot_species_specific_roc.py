#!/usr/bin/env python3
"""plot_species_specific_roc.py

Self-contained script that overlays ROC curves for the three species-specific
potency models (E.coli, S.aureus, P.aeruginosa) and **immediately shows** 
the figure when run via `python` or `%run` in a Jupyter notebook.
...
Expected files (generated by the training scripts):
    potency_judge_ecoli/potency_judge_ecoli_test_results.csv
    potency_judge_saureus/potency_judge_saureus_test_results.csv
    potency_judge_paeruginosa/potency_judge_paeruginosa_test_results.csv
"""
...
# -----------------------------------------------------------------------------
# Public helpers for species-specific models
# -----------------------------------------------------------------------------

def plot_species_specific_roc_combined(models_root: Path | str = DEFAULT_MODELS_ROOT, *, out_path: Path | str | None = None):
    """Generate combined ROC curves for all three species-specific potency models in a multi-panel layout."""
    
    models_root = Path(models_root)
    print(f"Loading species-specific ROC data from: {models_root}")
    
    # Create figure with 3 subplots (1x3 grid)
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    
    # Define specifications for each species (all use XGBoost, same blue color)
    species_specs = [
        ("E.coli Antimicrobial Activity Classification", [
            ("XGBoost", "potency_judge_ecoli", "tab:blue"),
        ]),
        ("S.aureus Antimicrobial Activity Classification", [
            ("XGBoost", "potency_judge_saureus", "tab:blue"),
        ]),
        ("P.aeruginosa Antimicrobial Activity Classification", [
            ("XGBoost", "potency_judge_paeruginosa", "tab:blue"),
        ])
    ]
    
    # Plot each species in its own subplot
    for i, (species_title, specs) in enumerate(species_specs):
        print(f"Generating ROC plot for {species_title}...")
        _overlay_subplot(axes[i], models_root, specs, species_title)
        # Add clean subplot title - larger font, no bold
        axes[i].set_title(species_title, fontsize=14, fontweight='normal', pad=10)
    
    plt.tight_layout()
    
    # Save combined figure
    if out_path is None:
        out_path = DEFAULT_PLOTS_DIR / "roc_curves_species_specific.png"
    else:
        out_path = Path(out_path)
    
    out_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_path, dpi=300, bbox_inches='tight')
    print(f"Combined species-specific ROC curves saved to {out_path}")
...
# ------------------------------------------------------------
# Convenience: when executed (CLI or pasted) show species-specific overlays
# ------------------------------------------------------------

def _show_species_specific(models_root: Path = DEFAULT_MODELS_ROOT):
    """Display combined species-specific ROC overlays in multi-panel layout."""
    plot_species_specific_roc_combined(models_root)
    print("Saved combined species-specific ROC figure to", DEFAULT_PLOTS_DIR)
...
if __name__ == "__main__" and not _IN_IPY:
    p = argparse.ArgumentParser(description="Overlay ROC curves for species-specific AMP classification tasks and display the figures.")
    ...
    _show_species_specific(models_dir)

# -----------------------------------------------------------------------------
# Auto-run in three situations -------------------------------------------------
#   1) `%run plot_species_specific_roc.py`  →  __name__ == '__main__' & not _IN_IPY
#   2) Paste entire file in a notebook cell  →  __name__ == '__main__' & _IN_IPY
#   3) `import plot_species_specific_roc` inside notebook  →  __name__ != '__main__' & _IN_IPY
...
if (_IN_IPY and __name__ == "__main__"):
    ...
    try:
        _show_species_specific(Path(DEFAULT_MODELS_ROOT))
    except Exception as err:
        print("plot_species_specific_roc –", err)
